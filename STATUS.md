...
| Jepsen | live register workload and Knossos checker | Sentinel `valid? true`; 60 s / 5,763 ops / 957 completed, under repeated total packet loss on one node; raw history in `jepsen/store/` |
...
- Jepsen/Knossos register history under repeated total packet loss on one node:
  `valid? true` over 5,763 invoked operations of which 957 completed; faults are
  applied by the cluster script, not by a Jepsen nemesis, so they do not appear
  in the history. Two larger runs from the same session reported `:valid? false`
  and were traced to a register key reused across histories, not to consensus;
  see `jepsen/store/ANALYSIS.md`.
